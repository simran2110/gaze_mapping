...
def preprocessData(inputDir):
    ...
    os.system(cmd_str)
    print(f"Preprocessed files saved in {output_root}/preprocessed_files")
    ### cleanup
...
def copyTobiiRecording_old(input_dir, output_root):
    """
...
def copyTobiiRecording(input_dir, output_root):
    ...
    # create a new output directory inside input folder
    # date_dir = dt.strftime('%Y_%m_%d')
    # time_dir = dt.strftime('%H-%M-%S')
    if not os.path.isdir(join(output_root, 'preprocessed_files')):
        os.makedirs(join(output_root, 'preprocessed_files'))
    outputDir = join(output_root, 'preprocessed_files')
...
def formatGazeData(input_dir):
    ...
    timeinseconds = raw_df['seconds'].values
    ...
    gaze_df = pd.DataFrame({'seconds':timeinseconds,
                            'timestamp': data_ts,
    ...
    colOrder = ['seconds', 'timestamp', 'frame_idx', 'confidence', 'norm_pos_x', 'norm_pos_y']
...
def json_to_df(json_file):
    ...
    with open(json_file, 'rb') as j:
        ...
        for key in sorted(vts_sync.keys()):
            ...
            df.loc[df.index >= key, 'vts_time'] = np.array(df.index)[df.index >= key]  # Necessary for multiple keys
        ...
        df['seconds'] = (df['index'] - df['index'][0])
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(args.inputDir):
        ...
        #python "/Users/simran/Desktop/SUMMER INTERNSHIP/Mapping Codes/mobileGazeMapping/mobileGazeMapping/preprocessing/tobii_preprocessing.py" ./ ./
        print("coming here")
        # run preprocessing on this data
        preprocessData(args.inputDir)
