...
def processRecording(gazeData=None, worldCameraVid=None, referenceImage=None, outputDir=None, nFrames=None):
    ...
    # Detect Markers for reference Image
    reference_image_pts = detect_markers_id_and_corner(refImgColor)
    logger.info('Detected makers Id and corners for reference Image')
    ...
    while vid.isOpened():
        ...
        if (ret is True) and (frameCounter in framesToUse):
            ...
            # fetch ids and corners for frame
            ...
            if len(frame_points) >= 4 and len(ref_points) >= 4:
                ...
                if homography is not None:
                    ...
                    for i, gazeRow in thisFrame_gazeData_world.iterrows():
                        ...
                        if world_gazeX == 0 or world_gazeY == 0:
            ...
            if not transformation_positive:
                print("Didnt Received markers, processing SIFT")
                # process this frame
...
if __name__ == '__main__':
    ...
    if input_folder is not None:
        outputDir = join(input_folder, 'mappedGazeOutput')
    else:
